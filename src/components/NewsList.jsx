...
const NewsList = ({ category }) => {
  /** usePromise 를 사용해 NewsList 에서 대기중 상태관리와 useEffect 설정을 직접하지않아도되며
   * --코드가 훨씬 간결해짐
   * 요청 상태를 관리할때 무조건 커스텀 훅을 만들어서 사용하는것은 아니지만 상황에따라 적절히 사용하면 좋은 코드를 만들어갈수있음*/
  const [loading, response, error] = usePromise(() => {
    const query = category === 'all' ? '' : `&category=${category}`;
    return axios.get(
        `https://newsapi.org/v2/top-headlines?country=kr${query}&apiKey=0d740fbe0f94426295a62879ccbae82c`
    )
    // usePromise 의 인수 deps 배열로 넘어갈 category
  }, [category])
